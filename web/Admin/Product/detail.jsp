...
<div class="wrapper bg-white mt-sm-5">
    ...
    <div class="py-1">
            ...
            <div class="col-md-6"> 
                ...
                <select name="category" id="country" class="bg-light">
                    <c:forEach items="${listCategory}" var="o">
                        <option value="${o.catID}" <c:if test="${product.catId == o.catID}">selected</c:if>>${o.catName}</option>
                    </c:forEach>
                </select> 
            </div>
        </div>
                ...
                <div class="arrow"> 
                    <select name="brand" id="language" class="bg-light" >
                        <c:forEach items="${listBrand}" var="o">
                            <option value="${o.bID}" <c:if test="${product.brandID == o.bID}">selected</c:if> >${o.bName}</option>
                        </c:forEach>
                    </select> 
                </div>
            </div>
        ...
        <div class="py-3 pb-4 border-bottom">
            <button type="submit" class="btn btn-primary mr-3" name="action" value="Save Changes">Save Changes</button> 
